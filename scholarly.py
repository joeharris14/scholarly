...
_KEYWORDSEARCH_CHOOSESTART = '/citations?view_op=search_authors&hl=en&mauthors=label:{0}&astart={1}'
_PUBSEARCH = '/scholar?q={0}'
_PUBSEARCH_CHOOSESTART = '/scholar?q={0}&start={1}'
...
def search_pubs_query_random(query):
    """Search by scholar query, but starting on a random page and return a generator of Publication objects"""
    url = _PUBSEARCH_CHOOSESTART.format(requests.utils.quote(query), random.randint(0, 40))
    soup = _get_soup(_HOST+url)
    return _search_scholar_soup(soup)
